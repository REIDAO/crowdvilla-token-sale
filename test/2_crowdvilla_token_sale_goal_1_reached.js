...
contract('All', function(accounts) {
  ...
  it("CrowdvillaTokenSale - Account #4 | Promised CRV should be " + contributionInEther * 400 + " CRV", function() {
    ...
    .then(function(result) {
      CRVmintedForContributors.push(result.valueOf());
      assert.equal(result.valueOf(), contributionInEther * (400 + (Math.max((bonusMultiplier-2),0) * 40)) * Math.pow(10,8));
    })
    ;
  });
  it("CrowdvillaTokenSale - Summary | MgmtFeeTokenAmount should be 860 CRV", function() {
